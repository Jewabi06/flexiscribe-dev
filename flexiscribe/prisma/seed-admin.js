...
async function main() {
  ...
  try {
    ...
    const hashedPassword = await bcrypt.hash("PSuaia##0127", 12);
    ...
    if (existingAdmin.rows.length > 0) {
      ...
      const result = await pool.query(
        ...
        ['psuia0127@gmail.com', hashedPassword, 'ADMIN']
      );
    ...
    if (existingAdminProfile.rows.length > 0) {
      ...
      const result = await pool.query(
        ...
        ['psuia0127', 'System Admin', adminUser.id]
      );
    ...
    if (parseInt(activityCheck.rows[0].count) === 0) {
      await pool.query(
        `INSERT INTO "Activity" (id, action, description, "userRole", "userName", "userId", "createdAt") 
         ...
         (gen_random_uuid(), 'User Login', 'Admin logged into the system', 'ADMIN', 'System Admin', $1, NOW() - INTERVAL '1 hour')`,
        [adminUser.id]
      );
    ...
    if (parseInt(auditCheck.rows[0].count) === 0) {
      await pool.query(
        `INSERT INTO "AuditLog" (id, action, details, "userRole", "userName", "adminId", "userId", "createdAt") 
         VALUES 
         (gen_random_uuid(), 'System Setup', 'Initial system configuration completed', 'ADMIN', 'System Admin', $1, $2, NOW()),
         (gen_random_uuid(), 'Admin Login', 'Administrator logged into the system', 'ADMIN', 'System Admin', $1, $2, NOW() - INTERVAL '10 minutes')`,
